...
#include "media/h265-camera-source.h"
...
static H265CameraSource *camera_source = NULL;
struct rtsp_media_t
{
...
static int rtsp_ondescribe(void* /*ptr*/, rtsp_server_t* rtsp, const char* uri)
{
	...
	char buffer[1024] = { 0 };
	{
		...
		if(it == s_describes.end())
		{
			...
			if (0 == strcmp(filename.c_str(), "") || 0 == strcmp(filename.c_str(), "camera"))
			{
#if defined(_HAVE_FFMPEG_)
#error 			"not support ffmpeg now!"
				source.reset(new FFLiveSource("video=Integrated Webcam"));
#endif
				camera_source = new H265CameraSource(filename.c_str());
	...
	// printf("sdp(%d):\n%s\n", strlen(sdp.c_str()), sdp.c_str());

	const char *new_sdp = "v=0\n"
...
"a=fmtp:100 profile-level-id=010C01;packetization-mode=1;sprop-parameter-sets=QAEMAf//AWAAAAMAAAMAAAMAAAMAlqwJAAAAAQ==";

	// printf("new sdp(%d):\n%s\n", strlen(new_sdp), new_sdp);
	// return rtsp_server_reply_describe(rtsp, 200, sdp.c_str());
	return rtsp_server_reply_describe(rtsp, 200, new_sdp);
}
...
void rtsp_send_h265_data(uint8_t *data, size_t data_len)
{
	TSessions::iterator it;
	for (it = s_sessions.begin(); it != s_sessions.end(); ++it)
	{
		rtsp_media_t& session = it->second;
		H265CameraSource *media = camera_source;
		if(session.status == 1) {
			media->SetNextFrame((uint8_t *)data, data_len);
		}
	}
}
